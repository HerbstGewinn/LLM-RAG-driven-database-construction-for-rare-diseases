...
from mapping import get_chemi_id, get_orpha_omim, get_pubmed_metadata, extract_metadata, get_EudraCT_metadata,get_clinicaltrials_metadata,get_hpos
...
def map_metadata(llm_output, file_path, analyzer, llm):
    ...
    if "EudraCT" in os.path.splitext(os.path.basename(file_path))[0]:
        ...
        if not study_data:
            ...
            if not study_data: # none of the three publication databases
                ...
                instructions = '''
                    You are an information extractor. Extract key meta-information from the DOCUMENT accurately.
                    Focus on These Elements: 
                        -Study Identifier (pub_code): A unique code assigned to the study.  
                        -DOI (pub_doi): The document’s Digital Object Identifier.  
                        -Year of Publication (pub_year): The year the study was published. This may follow keywords such as "Publication".
                        Extract only the year (e.g., 1975, 2010, 2018).
                        -Author Names (pub_authors):  This can be an individual's name or the sponsor, which is often a pharmaceutical company (e.g., Bayer, Lilly, AstraZeneca).
                        Sponsors are often mentioned after the keyword "sponsor". Keep it concise.
                        -Title of the Document (pub_title): The title often follows keywords like "title". The title is usually the main heading of the PDF.
                        It must not be edited, shortened, or modified in any way. Return it exactly as you find it in the file.
                        The extracted title should accurately reflect the study's content within its full context.
                        Titles typically include study design details (e.g., "Phase II, Randomized, Placebo-controlled Trial").
                        -Journal (pub_journal): Leave this field empty.
                    Important Guidelines:
                        -Do not hallucinate: If an element is missing or uncertain, leave it blank. Only extract information if you are very sure.
                        -Ensure accuracy and consistency in extracting information.
                        -The examples mentioned above are for illustrative purposes only. They should not be used in the output unless they are explicitly mentioned in the file.
                '''
                '''
                instructions = """
                ...
                if docs == "timeout":
                    ...
                    study_data = extract_metadata(docs_txt, llm, instructions,file_path)
    for key in study_data:
        if study_data.get(key, "N/A") == "N/A":
            '''
            instructions = f"""
                You are an information extractor.
            ...
            '''
            instructions = f"""
                You are an information extractor. Your task is to identify and extract only the {key} from the DOCUMENT.  
                The possible keys and their meanings are:  
                        -Study Identifier (pub_code): A unique code assigned to the study.  
                        -DOI (pub_doi): The document’s Digital Object Identifier.  
                        -Year of Publication (pub_year): The year the study was published. This may follow keywords such as "Publication".
                        Extract only the year (e.g., 1975, 2010, 2018).
            ...
            if docs == "timeout":
                ...
                docs_txt = analyzer.format_docs(docs)
                study_data = extract_metadata(docs_txt, llm, instructions,file_path)
        llm_output[key] = study_data.get(key, "N/A")

    print("---EXTRACTED STUDY METADATA---")
    print("Mapping extracted information...")
    ...
    if "disease" in llm_output:       
        ...
        for diseases in llm_output["disease"]:
            ordo_omim = get_orpha_omim(diseases)
            
            ordo_code.append(ordo_omim["Orpha_Code"]if ordo_omim else None)
            ordo_term.append(ordo_omim["Orpha_Name"]if ordo_omim else None)
            omim_code.append(ordo_omim["OMIM_Code"]if ordo_omim else None) 
            omim_name.append(ordo_omim["Omim_Name"]if ordo_omim else None) 
            
            llm_output["ORDO_code"] = ordo_code if ordo_code else "None"
            llm_output["ORDO_term"] = ordo_term if ordo_term else "None"
            llm_output["OMIM_Code"] = omim_code if omim_code else "None"  
            llm_output["OMIM_Name"] = omim_name if omim_name else "None" 
    else:
...
def complete_retrieval(llm_output):
    ...
    try:
        ...
        df = pd.DataFrame(columns=["Publication_database", "Study_identifier",
                                ...
                                "ORDO_code","HPO_terms_condition","Molecular_diagnosis_OMIM","OMIM_code","gene", "treatment",
    ...
    new_row = {
        ...
        "Molecular_diagnosis_OMIM": llm_output["OMIM_Name"] if "OMIM_Name" in llm_output else "None",
        "OMIM_code": llm_output["OMIM_Code"] if "OMIM_Code" in llm_output else "None",
...
def main():
    ...
    follow_up_querys = [
           """The paper addresses the following disease: {answer}.  
            ...
            Evaluate each treatment option's effect based on statements or assessments found in the the paper and classify it into one of the following categories:
            n/a (not applicable or not available), None, small, medium, or large. Return the classification for each treatment separately."""
            ]
